...
Since most `views/admin/` code (including RunSignUp/Athlinks/etc. integrations) has no pytest coverage as a result, verify it manually instead: `docker compose exec app flask shell` gives an interactive shell inside the running app container for exercising API-calling functions directly (e.g. `get_runsignup_client()` against a real race id), and the dev server in a browser covers anything routing/UI-dependent. `/admin/downloadresults` is a concrete example: pasting a real RunSignUp results-page URL exercises `AjaxDownloadResults.get()` (Year/Distance/Result Set dropdowns populate on success), and submitting exercises `DownloadResults.post()` (downloads the CSV) — both go through `get_runsignup_client()`.
...
**Which top-level modules are actually importable under the bare-`Flask` fixture** (verified by import, not just inspection): `model.py`, `resultsutils.py`, `clubmember.py`, `helpers.py`, `raceresults.py`, `athlinksresults.py`, `ultrasignupresults.py`, `runningaheadresults.py`, and `accesscontrol.py` (its `Principal(app)` call is fine with `app=None` — flask-principal defers init when no app is passed). `tools.py` is *not* importable this way despite living at the top level, not `views/admin/` — it does `from .views.admin.member import normalizeRAmemberlist`, which pulls in the same `celery.py` chain described above. Import-test a new top-level module before assuming it's fixture-testable; being outside `views/admin/` isn't sufficient on its own. `test/test_model.py`, `test/test_resultsutils.py`, `test/test_clubmember.py`, `test/test_helpers.py`, `test/test_raceresults.py`, `test/test_accesscontrol.py`, and `test/test_athlinksresults.py`/`test_ultrasignupresults.py`/`test_runningaheadresults.py` cover these.

Writing that coverage surfaced two pre-existing bugs, left unfixed (out of scope for a test-writing pass) but documented in the relevant test files:
- `resultsutils.ServiceAttributes.set_attr()` queries `RaceResultService` by a `name` kwarg the model doesn't have, and reads `self.service`, which `__init__` never sets — it would raise on any call. Nothing in the codebase currently calls it.
- `athlinksresults.AthlinksCollect.getresults()`'s age-group "fuzzy match" branch uses `(x/5)*5 != x`-style checks that assumed Python 2's truncating `/`. Under Python 3 true division, `(n/5)*5` reconstructs any int `n` exactly, so the "is this an age-group value" check never rejects, and the "does the runner's age fall in that group" check never matches (it recomputes the runner's own age, not the result's). Net effect: any age mismatch against an Athlinks result is skipped outright — fuzzy-age matching hasn't actually worked since the Python 3 port.
...
There are two separate MySQL connection strings in this codebase, and the `mysql+pymysql://` scheme has to be set on both independently: the main app DB URI in `settings.py` above, and Celery's result-backend URI in [`celery.py`](app/src/rrwebapp/celery.py)'s `celeryconfig['result_backend']` (`db+mysql+pymysql://...`, not `db+mysql://...`). Missing the second one doesn't just break the status-polling view (`AsyncResult.state`/`.info` in `/admin/importresultsstatus/<task_id>`, `results.py`) — it's worse from inside the task itself: `importresultstask` in [`tasks.py`](app/src/rrwebapp/tasks.py) calls `self.update_state(state='PROGRESS', ...)` periodically *inside* its own outer `try:` block, and that call is what first opens the result-backend connection. When it raises `ModuleNotFoundError: No module named 'MySQLdb'`, it's caught by the task's bare `except:`, which does `db.session.rollback()` — silently discarding every result row accumulated so far — then emails admins and returns a fake "success" response with the traceback embedded. So this bug can silently roll back entire results imports, not merely fail to report their progress.
...
- **RunSignUp is not part of the age-grade "Results Analysis" pipeline** — `/admin/resultsanalysisstatus`'s collect action (`analyzeresultstask` in [`tasks.py`](app/src/rrwebapp/tasks.py)) only wires up `collectservices` for `'athlinks'`, `'ultrasignup'`, and `'runningahead'` ([tasks.py:44-80](app/src/rrwebapp/tasks.py#L44-L80)); there's no `'runsignup'` entry, and the `RESULTS_ANALYSIS_DEBUG_*` env vars above cover only those three services. A RunSignUp-focused change (e.g. to `get_runsignup_client()`) doesn't affect that page.

The lookup pattern is `ApiCredentials.query.filter_by(name=servicename).first()` — see [`athlinksresults.py`](app/src/rrwebapp/athlinksresults.py) for the simplest example, or `get_runsignup_client()` in [`resultsutils.py`](app/src/rrwebapp/resultsutils.py) (the single choke point for both RunSignUp call sites in [`results.py`](app/src/rrwebapp/views/admin/results.py): `DownloadResults.post()` and `AjaxDownloadResults.get()`). Deliberately placed in `resultsutils.py` rather than `results.py` itself — see the Testing section's `celery.py` gotcha above for why.

`get_runsignup_client(anonymous=False, **kwargs)` — both `results.py` call sites now pass `anonymous=True` deliberately: since credentials don't gate access to results content (see the premise note below), there's no reason to send `key`/`secret`. `api_reg_token`/`api_reg_secret` are still sent whenever the `'runsignup'` `ApiCredentials` row has them, regardless of `anonymous`, since those are independent of `key`/`secret` in `RunSignupBase.__init__` (validated separately, no cross-dependency) and RunSignUp requires registration for all callers per its docstring, authenticated or not. `anonymous` defaults to `False` for callers that do need authenticated access (currently none in `results.py`, but e.g. a hypothetical future results-download feature needing `key`/`secret`-gated data would omit the flag).

**On the "credentials reveal fuller names" premise**: the original 2021 code comment this change was based on claimed RunSignUp shows minors' full names only to authenticated callers. A same-race CSV diff between anonymous and authenticated `DownloadResults` output (v4.2.5 vs. v4.3.0.dev1, compared 2026-08-07) showed the *only* difference was RunSignUp's placeholder for registrants who opted into its "Anonymous" privacy setting: `last_name` changes from `Anonymous` (unauthenticated) to `Participant` (authenticated) — a cosmetic placeholder swap, not a real name being revealed either way. No minor's actual last name changed between the two runs in that test. Don't assume authenticating actually surfaces real minor names without further verification against a race known to have masked-minor rows specifically (as opposed to "Anonymous"-flagged rows, which are a different RunSignUp privacy mechanism). RunSignUp support directly confirmed this (2026-08-08): API registration "doesn't authenticate you to receive full results, it just registers who the call is from" — consistent with the installed `running.runsignup` library's own docstrings, where every method `results.py` calls (`getrace`, `getraceevents`, `getresultsets`, `geteventresults`) is annotated `does not require credentials (userpriv=True)`. `rsu.members()` (used only by the separate membership-sync flow) is the one method that does require credentials.

A follow-up diff (v4.3.0.dev1+, generated with the `anonymous=True` code above, i.e. `key`/`secret` omitted but `api_reg_token`/`api_reg_secret` still sent) came out byte-identical to the *authenticated* v4.3.0.dev1 run, not the fully-unregistered v4.2.5 run — the `Anonymous`→`Participant` placeholder swap tracks **API registration**, not **authentication**. Matches `RunSignupBase.open()` ([runsignup.py:110-126](.venv/Lib/site-packages/running/runsignup.py#L110-L126)): the registration header/param are sent independent of `self.userpriv`. Still cosmetic, not a real name — but once RunSignUp's registration requirement goes live 2027-01-01, every caller (not just previously-authenticated ones) will see `Participant` instead of `Anonymous` for these opted-in rows, since registration is now effectively mandatory for everyone.
